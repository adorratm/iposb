...
class Product_categories extends MY_Controller
{
    ...
    {
        $items = $this->product_category_model->getRows([], $_POST);
        $data = [];
        ...
        if (checkEmpty($data)["error"] && checkEmpty($data)["key"] != "top_id") :
            ...
            if (!empty($_FILES)) :
                ...
                if (empty($_FILES["home_url"]["name"])) :
                    echo json_encode(["success" => false, "title" => "Başarısız!", "message" => "Ürün Kategorisi Eklenirken Hata Oluştu. Ürün Kategorisi Anasayfa Yatay Görseli Seçtiğinizden Emin Olup, Lütfen Tekrar Deneyin."]);
                    die();
                endif;
                if (empty($_FILES["banner_url"]["name"])) :
                    echo json_encode(["success" => false, "title" => "Başarısız!", "message" => "Ürün Kategorisi Eklenirken Hata Oluştu. Ürün Kategorisi Banner Görseli Seçtiğinizden Emin Olup, Lütfen Tekrar Deneyin."]);
                    die();
                endif;
    ...
    public function update_form($id)
    {
        ...
        $category = $this->product_category_model->get_all(["id!=" => $viewData->item->id]);
    ...
    public function parts($category_id)
    {
        ...
        $viewData->item = $this->product_category_model->get(["id" => $category_id]);
        $viewData->viewFolder = $this->viewFolder;
    ...
    {
        $items = $this->product_category_part_model->getRows(["category_id" => $category_id], $_POST);
        $data = [];
        $i = (!empty($_POST['start']) ? $_POST['start'] : 0);
        if (!empty($items)) :
            foreach ($items as $item) :
        ...
        $output = [
            "draw" => (!empty($_POST['draw']) ? $_POST['draw'] : 0),
            "recordsTotal" => $this->product_category_part_model->rowCount(["category_id" => $category_id]),
            "recordsFiltered" => $this->product_category_part_model->countFiltered(["category_id" => $category_id], (!empty($_POST) ? $_POST : [])),
            "data" => $data,
        ];
        // Output to JSON format
        echo json_encode($output);
    ...
    public function partSave()
    {
        ...
        if (checkEmpty($data)["error"] && checkEmpty($data)["key"] != "price") :
            ...
            $getRank = $this->product_category_part_model->rowCount();
    ...
    {
        $viewData = new stdClass();
        $viewData->item = $this->product_category_part_model->get(["id" => $id]);
        $viewData->viewFolder = $this->viewFolder;
    ...
    {
        $categoryPart = $this->product_category_part_model->get(["id" => $id]);
    ...
    {
        $items = $this->product_category_dimension_model->getRows(["category_id" => $category_id], $_POST);
        $data = [];
        $i = (!empty($_POST['start']) ? $_POST['start'] : 0);
        if (!empty($items)) :
            foreach ($items as $item) :
        ...
        $output = [
            "draw" => (!empty($_POST['draw']) ? $_POST['draw'] : 0),
            "recordsTotal" => $this->product_category_dimension_model->rowCount(["category_id" => $category_id]),
            "recordsFiltered" => $this->product_category_dimension_model->countFiltered(["category_id" => $category_id], (!empty($_POST) ? $_POST : [])),
            "data" => $data,
        ];
        // Output to JSON format
        echo json_encode($output);
    ...
    public function dimensionsSave()
    {
        ...
        if (checkEmpty($data)["error"] && checkEmpty($data)["key"] != "width" && checkEmpty($data)["key"] != "height" && checkEmpty($data)["key"] != "depth") :
            ...
            $getRank = $this->product_category_dimension_model->rowCount();
            $data["title"] = stripslashes($data["title"]);
            $data["isActive"] = 1;
            $data["rank"] = $getRank + 1;
    ...
    {
        $viewData = new stdClass();
        $viewData->item = $this->product_category_dimension_model->get(["id" => $id]);
        $viewData->viewFolder = $this->viewFolder;
    ...
    {
        $productCategoryDimension = $this->product_category_dimension_model->get(["id" => $id]);
    ...
    {
        $categories = $this->general_model->get_all("product_categories", "id,title", "rank ASC");
        if (!empty($categories)) :
            foreach ($categories as $categoryKey => $categoryValue) :
                $categoryParts = $this->general_model->get_all("product_category_parts", null, "rank ASC", ["category_id" => $categoryValue->id]);
                if (!empty($categoryParts)) :
                    $productIds = $this->general_model->get_all("products_w_categories", "product_id", null, ["category_id" => $categoryValue->id]);
                    if (!empty($productIds)) :
                        $arrayOfProductIds = [];
                        ...
                        endforeach;
                        $products = $this->general_model->get_all("products", "id", "rank ASC", [], [], [], [], ["id" => $arrayOfProductIds]);
                        if (!empty($products)) :
                            foreach ($products as $productKey => $productValue) :
                                $productParts = $this->general_model->get_all("product_parts", null, "rank ASC", ["product_id" => $productValue->id]);
                                $getRank = $this->general_model->rowCount("product_parts");
    ...
    public function syncDimensions()
    {
        ...
        if (!empty($categories)) :
            foreach ($categories as $categoryKey => $categoryValue) :
                $categoryDimensions = $this->general_model->get_all("product_category_dimensions", null, "rank ASC", ["category_id" => $categoryValue->id]);
                if (!empty($categoryDimensions)) :
                    ...
                    if (!empty($productIds)) :
                        ...
                        if (!empty($products)) :
                            foreach ($products as $productKey => $productValue) :
                                $productDimensions = $this->general_model->get_all("product_dimensions", null, "rank ASC", ["product_id" => $productValue->id]);
                                $getRank = $this->general_model->rowCount("product_dimensions");
