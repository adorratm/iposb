...
class Products extends MY_Controller
{
    ...
    public function datatable()
    {
        ...
        if (!empty($items)) :
            foreach ($items as $item) :
                ...
                $data[] = [$item->rank, '<i class="fa fa-arrows" data-id="' . $item->id . '"></i>', $item->id, $deleteCheckbox, $item->title, number_format($item->price, 2), $item->company_code, $checkbox1, $checkbox2, $checkbox3, $checkbox4, turkishDate("d F Y, l H:i:s", $item->updatedAt), turkishDate("d F Y, l H:i:s", $item->sharedAt), $proccessing];
            endforeach;
        endif;
        $output = [
    ...
    public function save()
    {
        ...
        if (checkEmpty($data)["error"] && checkEmpty($data)["key"] !== "discount" && checkEmpty($data)["key"] !== "price" && checkEmpty($data)["key"] !== "external_url" && checkEmpty($data)["key"] !== "content" && checkEmpty($data)["key"] !== "company_code") :
    ...
    {
        $viewData = new stdClass();
        $viewData->variation_categories = $this->product_variation_category_model->get_all();
        $viewData->variations = $this->product_variation_model->get_all();
        $viewData->viewFolder = $this->viewFolder;
    ...
    {
        $items = $this->product_variation_group_model->getRows(["product_id" => $product_id], $_POST);
        $data = [];
        $i = (!empty($_POST['start']) ? $_POST['start'] : 0);
        if (!empty($items)) :
            foreach ($items as $item) :
        ...
        $output = [
            "draw" => (!empty($_POST['draw']) ? $_POST['draw'] : 0),
            "recordsTotal" => $this->product_variation_group_model->rowCount(["product_id" => $product_id]),
            "recordsFiltered" => $this->product_variation_group_model->countFiltered(["product_id" => $product_id], (!empty($_POST) ? $_POST : [])),
            "data" => $data,
        ];
        // Output to JSON format
        echo json_encode($output);
    ...
    {
        $viewData = new stdClass();
        $viewData->categories = $this->product_variation_category_model->get_all();
        $viewData->variations = $this->product_variation_model->get_all();
    ...
    public function variation_group_save($product_id)
    {
        ...
        if (checkEmpty($data)["error"] && checkEmpty($data)["key"] != "discount" && checkEmpty($data)["key"] != "vat" && checkEmpty($data)["key"] != "price" && checkEmpty($data)["key"] != "stockStatus" && checkEmpty($data)["key"] != "stock") :
            ...
            if ($product) :
                $getRank = $this->product_variation_group_model->rowCount();
                $data["title"] = stripslashes($data["title"]);
    ...
    public function variation_group_update_form($id, $product_id)
    {
        ...
        $viewData->item = $this->product_variation_group_model->get(["id"    => $id, "product_id" => $product_id]);
        $viewData->settings = $this->general_model->get_all("settings", null, null, ["isActive" => 1]);
        $this->load->view("{$viewData->viewFolder}/{$viewData->subViewFolder}/content", $viewData);
    }
    ...
    {
        $product_variation_category = $this->product_variation_group_model->get(["id" => $id]);
    ...
    public function fileUpdate($id, $product_id)
    {
        ...
        $viewData->categories = $this->product_variation_category_model->get_all();
        $viewData->variations = $this->product_variation_model->get_all();
        $viewData->groups = $this->product_variation_group_model->get_all(["product_id" => $product_id]);
    ...
    public function partDatatable($product_id)
    {
        ...
        $output = [
            "draw" => (!empty($_POST['draw']) ? $_POST['draw'] : 0),
            "recordsTotal" => $this->product_part_model->rowCount(["product_id" => $product_id]),
            "recordsFiltered" => $this->product_part_model->countFiltered(["product_id" => $product_id], (!empty($_POST) ? $_POST : [])),
            "data" => $data,
        ];
        // Output to JSON format
        echo json_encode($output);
    ...
    public function partSave()
    {
        ...
        if (checkEmpty($data)["error"]) :
            ...
            $getRank = $this->product_part_model->rowCount();
            $data["title"] = stripslashes($data["title"]);
            $data["isActive"] = 1;
            $data["rank"] = $getRank + 1;
    ...
    {
        $viewData = new stdClass();
        $viewData->item = $this->product_part_model->get(["id" => $id]);
        $viewData->viewFolder = $this->viewFolder;
    ...
    {
        $productPart = $this->product_part_model->get(["id" => $id]);
    ...
    {
        $categories = $this->general_model->get_all("product_categories", "id,title", "rank ASC", [], [], [], [], ["id" => $arrayOfCategoryIds]);
        if (!empty($categories)) :
            foreach ($categories as $categoryKey => $categoryValue) :
                ...
                if (!empty($categoryParts)) :
                    $product = $this->general_model->get("products", "id", ["id" => $product_id]);
                    if (!empty($products)) :
                        $productParts = $this->general_model->get_all("product_parts", null, "rank ASC", ["product_id" => $product->id]);
            ...
            if (!empty($categories)) :
                foreach ($categories as $categoryKey => $categoryValue) :
                    ...
                    if (!empty($categoryDimensions)) :
                        $product = $this->general_model->get("products", "id", ["id" => $product_id]);
                        if (!empty($product)) :
                            $productDimensions = $this->general_model->get_all("product_dimensions", null, "rank ASC", ["product_id" => $product->id]);
