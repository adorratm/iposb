...
class Coupons extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $items = $this->coupon_model->get_all();
    ...
    {
        $items = $this->coupon_model->getRows([], $_POST);
        $data = [];
        ...
        if (!empty($items)) :
            foreach ($items as $item) :
                ...
                $data[] = [$item->rank, '<i class="fa fa-arrows" data-id="' . $item->coupon_id . '"></i>', $item->coupon_id, $item->title, $item->discount, $item->minPrice, (!empty($item->full_name) ? $item->full_name : "<b class='text-danger'>Tüm Kullanıcılar Kullanabilir.<b>"), (!empty($item->email) ? $item->email : "<b class='text-danger'>Tüm Kullanıcılar Kullanabilir.<b>"),$item->lang, $checkbox, turkishDate("d F Y, l H:i:s",$item->startedAt), turkishDate("d F Y, l H:i:s",$item->finishedAt), turkishDate("d F Y, l H:i:s", $item->createdAt), turkishDate("d F Y, l H:i:s", $item->updatedAt), $proccessing];
            endforeach;
        endif;
        $output = [
            "draw" => (!empty($_POST['draw']) ? $_POST['draw'] : 0),
            "recordsTotal" => $this->coupon_model->rowCount(),
            "recordsFiltered" => $this->coupon_model->countFiltered([], (!empty($_POST) ? $_POST : [])),
            "data" => $data,
        ];
    ...
    public function new_form()
    {
        ...
        $viewData->categories = $this->coupon_model->get_all();
        $viewData->viewFolder = $this->viewFolder;
    ...
    public function save()
    {
        ...
        if (checkEmpty($data)["error"] && checkEmpty($data)["key"] != "user_id") :
            ...
            $getRank = $this->coupon_model->rowCount();
            $data["isActive"] = 1;
            $data["rank"] = $getRank + 1;
    ...
    public function update_form($id)
    {
        ...
        $viewData->item = $this->coupon_model->get(["id" => $id]);
        $viewData->settings = $this->general_model->get_all("settings", null, null, ["isActive" => 1]);
        $this->load->view("{$viewData->viewFolder}/{$viewData->subViewFolder}/content", $viewData);
    }
    ...
    {
        $coupon = $this->coupon_model->get(["id" => $id]);
        if (!empty($coupon)) :
