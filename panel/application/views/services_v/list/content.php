...
<script>
	$(document).ready(function() {
		$(document).on("click", ".createServiceBtn", function(e) {
			...
			createModal("#servicesModal", "Yeni Bölüm Ekle", "Yeni Bölüm Ekle", 600, true, "20px", 0, "#e20e17", "#fff", 1040, function() {
		...
		$(document).on("click", ".updateServiceBtn", function(e) {
			...
			$('#servicesModal').iziModal('destroy');
			let url = $(this).data("url");
			createModal("#servicesModal", "Bölüm Düzenle", "Bölüm Düzenle", 600, true, "20px", 0, "#e20e17", "#fff", 1040, function() {
				$.post(url, {}, function(response) {
					$("#servicesModal .iziModal-content").html(response);
					TinyMCEInit();
					flatPickrInit();
