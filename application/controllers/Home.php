...
class Home extends MY_Controller
{
    ...
    public function in_parent($in_parent = null, $position = null, $lang = null, $store_all_id = null)
    {
        ...
        if (in_array($in_parent, $store_all_id)) :
            ...
            $html .=  '<ul '.($position =="HEADER" ? 'id="backmenu" class="react-menus home react-sub-shadow"' : null).'>';
            foreach ($result as $key => $value) :
                ...
                $html .= '<li ' . (($position == "HEADER") && in_array($value->id, $store_all_id) ? ((!empty($page->url) && ($this->uri->segment(2) == strto("lower", seo($page->url)) || $this->uri->segment(3) == strto("lower", seo($page->url)))) || $this->uri->segment(2) == strto("lower", seo($value->title)) || $this->uri->segment(3) == strto("lower", seo($value->title)) || ($this->uri->segment(2) === null && $value->url === '/') ? "class='active'" : "active") : ((!empty($page->url) && ($this->uri->segment(2) == strto("lower", seo($page->url)) || $this->uri->segment(3) == strto("lower", seo($page->url)))) || ($this->uri->segment(2) === null && $value->url === '/') || $this->uri->segment(2) == strto("lower", seo($value->title)) || $this->uri->segment(3) == strto("lower", seo($value->title)) ? null : null)) . '>';
                if (empty($value->url)) :
    ...
    public function services()
    {
        ...
        $this->viewData->og_image           = clean(get_picture("settings_v", $this->viewData->settings->logo));
        $this->viewData->og_type          = "article";
        $this->viewData->og_title           = clean(strto("lower|upper", lang("routes_services"))) . " - " . $this->viewData->settings->company_name;
        $this->viewData->og_description           = clean($this->viewData->settings->meta_description);
        $this->viewData->links = $this->pagination->create_links();
