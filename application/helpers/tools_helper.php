...
function clean($data)
{
    ...
    if (!empty($data)) :
        ...
        return html_purify($t->security->xss_clean($data));
    endif;
    // we are done...
    return null;
}
...
function product_in_parent($in_parent = null, $checkedValues = null, $lang = null, $store_all_id = null)
{
    ...
    if (in_array($in_parent, $store_all_id)) :
        ...
        if (!empty($result)) :
            foreach ($result as $key => $value) :
                $html .= "<h3 class='text-dark-green mt-3'>" . $value->title . "</h3>";
                ...
                if (!empty($variations)) :
                    ...
                    foreach ($variations as $k => $v) :
                        if (in_array($v->id, $availableVariations)) :
                            ...
                            $html .= "      <input type='checkbox' class='me-3' style='width:25px!important;height:25px!important' name='selectedVariations[]' " . (in_array($v->id, $checkedValues) ? "checked" : null) . " value='" . $v->id . "'><b>" . $v->title . "</b>";
                            $html .= "  </div>";
                            $html .= "</li>";
                        endif;
                    endforeach;
