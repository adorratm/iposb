...
<div class="mb-3">
    <div class="card-body p-0">
        <div class="table-responsive">
            <table class="table table-hover table-striped table-bordered w-100">
                <thead>
                    <tr>
                        <th class="text-center align-middle" colspan="2"><strong><?= strto("lower|upper", lang("userInformation")) ?></strong></th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
            ...
            <table class="table table-hover table-striped table-bordered w-100">
                <thead>
                    <tr>
                        <th colspan="2" class="text-center align-middle"><strong><?= strto("lower|upper", lang("deliveryAndBillingAddress")) ?></strong></th>
                    </tr>
                </thead>
                <tr>
                    <td class="text-center align-middle">
            ...
            <?php if (!empty($order_data)) : ?>
                <table class="table table-bordered table-striped table-hover w-100">
                    <thead>
                        <tr>
                            <th class="font-weight-bold text-center align-middle" colspan="5"><?= strto("lower|upper", lang("orderDetail")) ?></th>
                        </tr>
                        <tr>
                        ...
                        <?php foreach ($order_data["cart"] as $cart_key => $cart_value) : ?>
                            ...
                            <?php if (!empty($product)) : ?>
                                <tr>
                                    ...
                                    <td class='text-center align-middle'><?= $cart_value["price"] ?> <?= $item->symbol ?></td>
                                    <td class='text-center align-middle'><?= $cart_value["subtotal"] ?> <?= $item->symbol ?></td>
                                </tr>
                            <?php endif ?>
                        <?php endforeach ?>
                    </tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php switch ($item->shippingCompany):
                                    case "ARAS": ?>
                                        <a class="btn btn-primary d-flex text-center justify-content-center" title="<?= lang("shippingLink") ?>" target="_blank" href="https://kargotakip.araskargo.com.tr/mainpage.aspx?code=<?= (!empty($item->shippingCode) ? $item->shippingCode : null) ?>"><?= lang("shippingLink") ?></a>
                                        <?php break ?>
                                    <?php
                                    case "MNG": ?>
                                        <a class="btn btn-primary d-flex text-center justify-content-center" title="<?= lang("shippingLink") ?>" target="_blank" href="https://service.mngkargo.com.tr/iactive/popup/KargoTakip/link1.asp?k=<?= (!empty($item->shippingCode) ? $item->shippingCode : null) ?>"><?= lang("shippingLink") ?></a>
                                        <?php break ?>
                                    <?php
                                    case "YURTİÇİ": ?>
                                        <a class="btn btn-primary d-flex text-center justify-content-center" title="<?= lang("shippingLink") ?>" target="_blank" href="https://selfservis.yurticikargo.com/reports/SSWDocumentDetail.aspx?DocId=<?= (!empty($item->shippingCode) ? $item->shippingCode : null) ?>"><?= lang("shippingLink") ?></a>
                                        <?php break ?>
                                    <?php
                                    case "SÜRAT": ?>
                                        <a class="btn btn-primary d-flex text-center justify-content-center" title="<?= lang("shippingLink") ?>" target="_blank" href="https://suratkargo.com.tr/KargoTakip/?kargotakipno=<?= (!empty($item->shippingCode) ? $item->shippingCode : null) ?>"><?= lang("shippingLink") ?></a>
                                        <?php break ?>
                                    <?php
                                    default: ?>
                                        <a class="btn btn-primary d-flex text-center justify-content-center" title="<?= lang("shippingLink") ?>" target="_blank" href="https://service.mngkargo.com.tr/iactive/popup/KargoTakip/link1.asp?k=<?= (!empty($item->shippingCode) ? $item->shippingCode : null) ?>"><?= lang("shippingLink") ?></a>
    ...
    <a href="javascript:void(0)" onclick="closeModal('#ordersModal')" title="<?= lang("close") ?>" class="btn btn-sm btn-outline-danger rounded-0"><?= lang("close") ?></a>
</div>